...
--BlxFrtVars.ForceMode = "" -- can be Empty/Unsafe/Safe (Empty = auto, Unsafe = always shamblesTo(), Safe = always moveTo())
...
local function shamblesTo(part1, part2)
    ...
    local a = TweenInfo.new(math.abs((part2.Position- part1.Position).Magnitude) / char.Humanoid.WalkSpeed,Enum.EasingStyle.Quart,Enum.EasingDirection.Out,0,false,0)
...
AI.SmartMove = function(targetPart)
    ...
    if (targetPart.Position - hrp.Position).Y <= 10 or _G.FORCESAFE then
...
local jsCFG = HttpService:JSONDecode(game:HttpGet("https://raw.githubusercontent.com/teppyboy/RbxScripts/master/Misc/ZeroFuruit/ZeroFuruitCfg.json"))
